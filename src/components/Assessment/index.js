...
const Assessment = () => {
  ...
  const submitAssesment = () => {
    const score = selectedAnswers.filter(each => each.is_correct === 'true')
      .length
    const timeTaken = 600 - time
    history.replace('/results', {timeTaken, score})
  }
  ...
  const checkifOptionIsSelected = option => {
    ...
    if (filteredArr[0] !== undefined) {
      ...
      setSelectedAnswers([...selectedAnswers, {...option, qNo: questionNumber}])
    }
  }
